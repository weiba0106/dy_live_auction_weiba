...
public class OrderServiceImpl implements OrderService {
    ...
    public Order createOrder(Long itemId, Long buyerId) {
        Order existing = findByItemId(itemId);
        ...
        if (item.getStatus() != 4) throw new BizException(409, "竞拍尚未成交，不能生成订单");
        if (!buyerId.equals(item.getWinnerId())) throw new BizException(403, "买家不是竞拍赢家");
        ...
        try {
            ...
            Order concurrent = findByItemId(itemId);
    ...
    public Order payOrder(Long orderId, Long buyerId) {
        ...
        int settled = userMapper.settleFrozenFunds(buyerId, order.getFinalPrice());
        if (settled != 1) throw new BizException(ErrorCode.BALANCE_INSUFFICIENT);

        int paid = orderMapper.markPaidIfPending(orderId, buyerId);
        if (paid != 1) throw new BizException(ErrorCode.ORDER_ALREADY_PAID);
    ...
    private Order findByItemId(Long itemId) {
